...
controls = { --set your controls here :), make sure its all uppercase
	...
	{'left', 'right'},
	{'left', 'SPACE', 'RIGHT'},
	{'left', 'down', 'up', 'right'}, --lower case here for built in controls, otherwise do upper
...
function goodNoteHit(id, direction, noteType, isSustainNote) --add note type stuff here
	...
	if botPlay then --botplay stuff
		if active then
			direction = dataConvertFromType(noteType, direction%4);
			doStrumAnim(1, direction, 'confirm')
			if not isSustainNote then 
				spawnNoteSplash(direction);
			end
		end
	end
...
local sDir = {'singLEFT','singDOWN','singUP','singRIGHT','singUP','singLEFT','singDOWN','singUP','singRIGHT'} -- anim names for characters
local bfsDir = {'singLEFT','singDOWN','singUP','singRIGHT','singSPACE','singLEFT','singDOWN','singUP','singRIGHT'}
...
noteScales = {0.7,0.7,0.7,0.7,0.7,0.65,0.6,0.55,0.55,0.46};
noteWidths = {140,140,126,119,112,91,84,77,70,63};
...
function onUpdatePost(elapsed)
	...
	if active then 
		...
		for i = 0,8 do --set bools for controls
			...
			elseif controlsManiaConvert[p1curMania + 1][i + 1] == 'left' then -- to use built in keybinds
				p1strums[i..'.press'] = keyJustPressed('left');
				p1strums[i..'.release'] = keyReleased('left');
				p1strums[i..'.held'] = keyPressed('left');
			elseif controlsManiaConvert[p1curMania + 1][i + 1] == 'down' then 
				p1strums[i..'.press'] = keyJustPressed('down');
				p1strums[i..'.release'] = keyReleased('down');
				p1strums[i..'.held'] = keyPressed('down');
			elseif controlsManiaConvert[p1curMania + 1][i + 1] == 'up' then 
				p1strums[i..'.press'] = keyJustPressed('up');
				p1strums[i..'.release'] = keyReleased('up');
				p1strums[i..'.held'] = keyPressed('up');
			elseif controlsManiaConvert[p1curMania + 1][i + 1] == 'right' then 
				p1strums[i..'.press'] = keyJustPressed('right');
				p1strums[i..'.release'] = keyReleased('right');
				p1strums[i..'.held'] = keyPressed('right');
			else 
		...
		for i = 0, noteCount-1 do
			if getPropertyFromGroup('notes', i, 'mustPress') then
				...
				local tooLate = getPropertyFromGroup('notes', i, 'tooLate');
				local wasGoodHit = getPropertyFromGroup('notes', i, 'wasGoodHit');
				local strumTime = getPropertyFromGroup('notes', i, 'strumTime');
				local hitHealth = getPropertyFromGroup('notes', i, 'hitHealth');
				local noAnim = getPropertyFromGroup('notes', i, 'noAnimation');
				local gfNote = getPropertyFromGroup('notes', i, 'gfNote');

				noteData = dataConvertFromType(noteType, noteData);
				
				noteX = getProperty(noteData + (maxAmmount * 1)..'.x');
	
				setPropertyFromGroup('notes', i, 'x', noteX + (getPropertyFromGroup('notes', i, 'offsetX') / (0.7 / p1curScale))); --set note x to match with strum
				...
				setPropertyFromGroup('notes', i, 'x', noteX + (getPropertyFromGroup('notes', i, 'offsetX') / (0.7 / p2curScale))); --set note x to match with strum
	...
	for ii = 0, 50 do --doing a pool of 50 seems better for performance since it will reuse old lua sprites which could sometimes lag the game
...
function onBeatHit()
	...
	if (curBeat % getProperty('boyfriend.danceEveryNumBeats') == 0 and not string.find(string.lower(getProperty('boyfriend.animation.curAnim.name')), 'sing')) then 
		characterDance('boyfriend'); --fix idle when stunned
	end
end
...
function noteHit(i, noteData, noteType, isSustainNote, noAnim, GFnote)
	...
	triggerEvent('luaNoteHit', tostring(getPropertyFromGroup('notes', i, 'noteData')), tostring(isSustainNote))
end
...
function keyPress(data) --redoin inputs and shit

	local noteIdxToHit = -1
	local lowestStrumTime = -1
	local hittableNotes = 0
	for i = 0, noteCount-1 do
		if getPropertyFromGroup('notes', i, 'mustPress') then
			local isSustainNote = getPropertyFromGroup('notes', i, 'isSustainNote');
			local noteData = getPropertyFromGroup('notes', i, 'noteData');
			...
			local wasGoodHit = getPropertyFromGroup('notes', i, 'wasGoodHit');
			local strumTime = getPropertyFromGroup('notes', i, 'strumTime');
			local noteData = dataConvertFromType(noteType, noteData);
			if canBeHit and not tooLate and not wasGoodHit and not isSustainNote and noteData == data then --get notes that can be hit

				if lowestStrumTime == -1 then 
					noteIdxToHit = i 
					lowestStrumTime = strumTime
				elseif strumTime <= lowestStrumTime then 
					noteIdxToHit = i 
					lowestStrumTime = strumTime
				end
				hittableNotes = hittableNotes + 1
	...
	if noteIdxToHit > -1 then --hit le note
		isSustainNote = getPropertyFromGroup('notes', noteIdxToHit, 'isSustainNote');
		noteData = getPropertyFromGroup('notes', noteIdxToHit, 'noteData');
		noteType = getPropertyFromGroup('notes', noteIdxToHit, 'noteType');
		canBeHit = getPropertyFromGroup('notes', noteIdxToHit, 'canBeHit');
		tooLate = getPropertyFromGroup('notes', noteIdxToHit, 'tooLate');
		wasGoodHit = getPropertyFromGroup('notes', noteIdxToHit, 'wasGoodHit');
		strumTime = getPropertyFromGroup('notes', noteIdxToHit, 'strumTime');
		hitHealth = getPropertyFromGroup('notes', noteIdxToHit, 'hitHealth');
		missHealth = getPropertyFromGroup('notes', noteIdxToHit, 'missHealth');
		causesMiss = getPropertyFromGroup('notes', noteIdxToHit, 'hitCausesMiss');
		noAnim = getPropertyFromGroup('notes', noteIdxToHit, 'noAnimation');
		gfNote = getPropertyFromGroup('notes', noteIdxToHit, 'gfNote');
		noteData = dataConvertFromType(noteType, noteData);

		addScore(350);
		addHits(1);
		setPropertyFromGroup('notes', noteIdxToHit, 'wasGoodHit',true)
		doNoteRating(strumTime, noteData);
		setProperty('health', getProperty('health') + (hitHealth * healthGainMult));
		noteHit(noteIdxToHit, noteData, noteType, isSustainNote,noAnim, gfNote);
		removeFromGroup('notes', noteIdxToHit);
		
		if hittableNotes > 1 then --check for stacked notes
			noteCount = getProperty('notes.length');
			for i = 0, noteCount-1 do
				if getPropertyFromGroup('notes', i, 'mustPress') then
					local isSustainNote = getPropertyFromGroup('notes', i, 'isSustainNote');
					...
					local noteData = dataConvertFromType(noteType, noteData);
					if canBeHit and not tooLate and not wasGoodHit and not isSustainNote and noteData == data then
						if (strumTime - lowestStrumTime) < 35 then --note just hit now shouldnt exist in the list
							--debugPrint('stacked note found')
							removeFromGroup('notes', i); --yeeeeeeeeee it finally works
						end
					end 
				end
		...
		if not ghostTapping then 
			missPress(data) --you miss pressed you fucking dumbass
		end
	end
...
function missPress(data) 
	addMisses(1)
	if not practice then 
		addScore(-10)
	end
	setProperty('health', getProperty('health') - (0.05 * healthLossMult));
	setProperty('vocals.volume', 0);
	setProperty('combo', 0);

	local char = 'boyfriend'
	if getProperty(char..'.hasMissAnimations') then 
		local animToPlay = sDir[data + 1]..'miss'
		characterPlayAnim(char, animToPlay, true);
	end
...
function spawnNoteSplash(data)
	...
	if getPropertyFromClass('ClientPrefs', 'noteSplashes') then
		...
		local frameRate = 24 + getRandomInt(-2, 2)

		makeAnimatedLuaSprite(splashName, 'sploosh', getProperty(strumname..'.x'), getProperty(strumname..'.y'));
		addAnimationByPrefix(splashName, 'splash', animName, frameRate, false);
		setGraphicSize(splashName, p1curWidth * 3)
		...
		setProperty(splashName..'.x', getProperty(splashName..'.x') - ((getProperty(splashName..'.width') - p1curWidth) / 2)); --offset
		setProperty(splashName..'.y', getProperty(splashName..'.y') - ((getProperty(splashName..'.height') - p1curWidth) / 2));
		addLuaSprite(splashName);
		splashCount = splashCount + 1;

		if splashCount > 50 then
			splashCount = 1
			--removedSplashCount = 0
		end
	end
end
...
function doStrumAnim(player, data, animname)
	...
	objectPlayAnimation(name, animname, true);
